	...
	if data.size() > 0:
		...
		match type:
			DMConstants.TYPE_DIALOGUE:
				...
				# If the line has an autoadvance tag, also make it unskippable
				if tags.has("autoadvance"):
					var is_already_unskippable := false
					# Check if an unskippable tag already exists
					for tag in tags:
						if tag.begins_with("unskippable="):
							is_already_unskippable = true
							break
					# If not, add it
					if not is_already_unskippable:
						tags.append("unskippable=true")
