...
@export var skip_action: StringName = &"ui_skip"
...
func _should_auto_pause() -> bool:
	...
	# Ignore pause if it's part of a chain and not the last in the chain
	var pause_chars: PackedStringArray = pause_at_characters.split()
	if parsed_text[visible_characters - 1] in pause_chars:
		# Check if the next character is also a pause character
		if visible_characters < parsed_text.length() and parsed_text[visible_characters] in pause_chars:
			return false
			
	# Pause on hyphen if preceded by a space
	if parsed_text[visible_characters - 1] == "-" and visible_characters >= 2 and parsed_text[visible_characters - 2] == " ":
		return true
