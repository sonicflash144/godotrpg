...
## A reference to the currently active balloon
...
func show_dialogue_balloon(resource: DialogueResource, title: String = "", balloon_path: String = "res://Dialogue/balloon.tscn", extra_game_states: Array = []) -> Node:
...
func _start_balloon(balloon: Node, resource: DialogueResource, title: String, extra_game_states: Array) -> void:
	# If a balloon is already on screen and valid, destroy the new one and ignore the request.
	if is_instance_valid(_active_balloon):
		balloon.queue_free()
		return

	# Set the active balloon and connect to its exit signal
	_active_balloon = balloon
	_active_balloon.tree_exited.connect(_on_balloon_exited)
